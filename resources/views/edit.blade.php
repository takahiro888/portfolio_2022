...
<div class="card">
  ...
  <form class="card-body my-card-body" action="{{ route('update')}}" method="POST">
    @csrf
    <input type="hidden" name="memo_id" class="form-control" value="{{ $edit_memo[0]['id'] }}" />
    <div class="form-group">
      <textarea class="form-control w-50 mb-3" name="content" rows="3" placeholder="ここにメモを入力">{{ $edit_memo[0]['content'] }}</textarea>
    </div>
    @error('content')
    ...
    <div class="form-check form-check-inline mb-3">
      ...
      value="{{ $t['id'] }}" {{ in_array($t['id'], $include_tags, true) ? 'checked' : '' }}>
